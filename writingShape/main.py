...
for cnt in contours:
	...
	if area > 1000:
		...
		y = approx.ravel()[1]
		if len(approx) == 3:
			cv2.putText(img, "Triangle", (x,y-10), font, 1, (0), thickness=2, lineType=None, bottomLeftOrigin=None)
			tri += 1
		elif len(approx) == 4:
			cv2.putText(img, "Rectangle", (x,y-10), font, 1, (0), thickness=2, lineType=None, bottomLeftOrigin=None)
			rec += 1
		elif len(approx) == 5:
			cv2.putText(img, "Pentagon", (x,y-10), font, 1, (0), thickness=2, lineType=None, bottomLeftOrigin=None)
			pen += 1
		elif 6 < len(approx) < 11:
			el += 1
			cv2.putText(img, "Ellipse", (x,y-10), font, 1, (0), thickness=2, lineType=None, bottomLeftOrigin=None)
		else:
			cir += 1
			cv2.putText(img, "Circle", (x,y-10), font, 1, (0), thickness=2, lineType=None, bottomLeftOrigin=None)
